...
namespace ASCOM.Setup
{
    public partial class LocalServerForm : Form
    {
        ...
        private const string csNamespaceFormat = "ASCOM.{0}.{1}"; // Format string for driver namespace.
        ...
        internal string Namespace
        {
            get
            {
                return String.Format(csNamespaceFormat, DeviceName, DeviceClass);
            }
        }
        ...
        /// <remarks>
        /// The device name must start with an alphabetic character and may be followed by any number of alphanumeric characters. There may be no whitespace or punctuation.
        /// </remarks>
        ...
        private void InitASCOMClasses()
        {
            ...
            if (CallingType == typeof(ASCOM.Setup.LocalServerWizard)) // Form called from local server wizard
            {
                ...
                // Add the interfaces to the drop-down list and pre-select the Telescope type
                foreach (KeyValuePair<string, ASCOMInterface> ai in interfaceList)
                {
                    cbDeviceClass.Items.Add(ai.Value.DeviceType);
                }

                // Select Telescope as the default
                cbDeviceClass.SelectedIndex = cbDeviceClass.FindString("Telescope");
            }
