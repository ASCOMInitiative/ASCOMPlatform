...
namespace ASCOM.Simulator
{
    ...
    public class ClassFactory : IClassFactory
    {
        ...
        {
            OCSimulator.TL.LogMessage("ClassFactory", "Start of initialisation");
            ...
            {
                OCSimulator.TL.LogMessage("ClassFactory", "Adding Type: " + T.FullName);
                m_InterfaceTypes.Add(T);
            }
            OCSimulator.TL.LogMessage("ClassFactory", "Completed initialisation");
        }
        ...
        public bool RegisterClassObject()
        {
            ...
            OCSimulator.TL.LogMessage("RegisterClassObject", "GUID: " + m_ClassId.ToString() + ", Cookie: " + m_Cookie);
            return (i == 0);
        }

        public bool RevokeClassObject()
        {
            OCSimulator.TL.LogMessage("RevokeClassObject", m_Cookie.ToString());
        ...
        {
            OCSimulator.TL.LogMessage("ResumeClassObjects", "Called");
        ...
        {
            OCSimulator.TL.LogMessage("SuspendClassObjects", "Called");
        ...
        {
            OCSimulator.TL.LogMessage("CreateInstance", "GUID: " + riid.ToString());
        ...
        {
            OCSimulator.TL.LogMessage("LockServer", "Lock server: " + bLock);
            if (bLock)
                Server.CountLock();
            else
                Server.UncountLock();
            // Always attempt to see if we need to shutdown this server application.
            OCSimulator.TL.LogMessage("LockServer", "Calling ExitIf to check whether we can exit the server");
            Server.ExitIf();
        }
        #endregion
    }
