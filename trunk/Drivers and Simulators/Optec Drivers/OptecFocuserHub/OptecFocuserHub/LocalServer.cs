//
// OptecFocuserHub Local COM Server
//
...
namespace ASCOM.OptecFocuserHub
{
    public class OptecFocuserHub
    {
        ...
        private static string m_sAppId = "{cea7f0bf-aff0-4bbe-a91a-cdd66a2e1e0f}";	// Our AppId
        #endregion
        ...
        // the OptecFocuserHub\bin\Debug subfolder.
        //
        private static bool LoadComObjectAssemblies()
        {
            ...
            int i = assyPath.LastIndexOf(@"\OptecFocuserHub\bin\");						// Look for us running in IDE
            if (i == -1) i = assyPath.LastIndexOf('\\');
            assyPath = assyPath.Remove(i, assyPath.Length - i) + "\\OptecFocuserHubServedClasses";

            DirectoryInfo d = new DirectoryInfo(assyPath);
            foreach (FileInfo fi in d.GetFiles("*.dll", SearchOption.AllDirectories))
            {
                ...
                catch (Exception e)
                {
                    MessageBox.Show("Failed to load served COM class assembly " + fi.Name + " - " + e.Message,
                        "OptecFocuserHub", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                    return false;
                }
                ...
                switch (args[0].ToLower())
                {
                    ...
                    default:
                        MessageBox.Show("Unknown argument: " + args[0] + "\nValid are : -register, -unregister and -embedding",
                            "OptecFocuserHub", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        break;
                }
            }
            else
