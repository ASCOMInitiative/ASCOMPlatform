...
double *readeph(int mp, char *name, double jd, int *err) {
	...
	*err = 0;
	...
	if (astinf.num == 0) {
		...
		if (*err != 0)
			return NULL;
		strcpy_s(astinf.name[0], strlen(name) + 1, name);
		strcpy_s(infile, strlen(name) + 6, name);
		strcat_s(infile, strlen(name) + 6, ".chby");
		if (fopen_s(&astinf.fp[0], infile, "rb") != 0) {
		...
		if (mpnum == astinf.num) {
			...
			if (*err != 0)
				return NULL;
			strcpy_s(astinf.name[mpnum], strlen(name) + 1, name);
			strcpy_s(infile, strlen(name) + 6, name);
			strcat_s(infile, strlen(name) + 6, ".chby");
			if (fopen_s(&astinf.fp[mpnum], infile, "rb") != 0) {
				fprintf(stderr, "Can not open Chebyshev input file for ");
				fprintf(stderr, "%s.\n", name);
				*err = 4;
				return NULL;
