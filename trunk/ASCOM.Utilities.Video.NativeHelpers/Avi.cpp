...
Copyright (c) 2004, 2005 Ren? Nyffenegger (adding C++ class features)
...
HRESULT Avi::compression(HBITMAP const& bmp, AVICOMPRESSOPTIONS *opts, bool ShowDialog, HWND hparent) {
	if (avi_ == 0) return AVIERR_BADHANDLE;

	DIBSECTION dibs;
	dibs.dsBmih.biSizeImage = 0;
	dibs.dsBmih.biWidth = 0;
	dibs.dsBmih.biHeight = 0;
	dibs.dsBmih.biSize = 0;
...
HRESULT Avi::add_frame(HBITMAP const& bmp) {
	if (avi_ == 0) return AVIERR_BADHANDLE;
...
unsigned int FormatAviMessage(HRESULT code, char *buf, unsigned int len) {
	...
	// strncpy(buf, msg, n); Changed by Peter Simpson to remove compiler warning C4996 which states that this version is insecure
