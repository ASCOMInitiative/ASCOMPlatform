...
namespace ASCOM.Astrometry.NOVAS
{
    ...
    public class NOVAS3 : INOVAS3, IDisposable
    {
        ...
        private const string NOVAS32DLL = "NOVAS3.dll"; // Names of NOVAS 32 and 64bit DLL files
        ...
        /// </summary>
        /// <exception cref="HelperException">Thrown if the NOVAS3 support library DLL cannot be loaded</exception>
        /// <remarks></remarks>
        public NOVAS3()
        {
            bool rc;
            ...
            if (Is64Bit()) // 64bit application so find the 32bit folder location
            {
                ...
                Novas3DllFile = ReturnedPath.ToString() + NOVAS_DLL_LOCATION + NOVAS64DLL;
            ...
            {
                Novas3DllFile = GetFolderPath(SpecialFolder.CommonProgramFiles) + NOVAS_DLL_LOCATION + NOVAS32DLL;
            ...
            TL.LogMessage("New", "Loading NOVAS3 library DLL: " + Novas3DllFile);

            Novas3DllHandle = LoadLibrary(Novas3DllFile);
            LastError = Marshal.GetLastWin32Error();

            if (Novas3DllHandle != IntPtr.Zero) // Loaded successfully
            {
                TL.LogMessage("New", "Loaded NOVAS3 library OK");
            }
            else // Did not load 
            {
                TL.LogMessage("New", "Error loading NOVAS3 library: " + LastError.ToString("X8"));
                throw new HelperException("Error code returned from LoadLibrary when loading NOVAS3 library: " + LastError.ToString("X8"));
            }
            ...
            rc1 = Ephem_Open(JPLEphFile, ref JPL_EPHEM_START_DATE, ref JPL_EPHEM_END_DATE);
            if (rc1 > 0)
                throw new HelperException("Unable to open ephemeris file: " + JPLEphFile + ", RC: " + rc1);

            TL.LogMessage("New", "NOVAS3 initialised OK");
        }
...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
        ...
        public short AppStar(double JdTt, CatEntry3 Star, Accuracy Accuracy, ref double Ra, ref double Dec)
        {
            if (Is64Bit())
            {
                return AppStar64(JdTt, ref Star, Accuracy, ref Ra, ref Dec);
            }
            else
            {
                return AppStar32(JdTt, ref Star, Accuracy, ref Ra, ref Dec);
            }
        }
        ...
        public short AstroStar(double JdTt, CatEntry3 Star, Accuracy Accuracy, ref double Ra, ref double Dec)
        {
            ...
            {
                return AstroStar32(JdTt, ref Star, Accuracy, ref Ra, ref Dec);
            }
        }
        ...
        public void Equ2Hor(double Jd_Ut1, double DeltT, Accuracy Accuracy, double x, double y, OnSurface Location, double Ra, double Dec, RefractionOption RefOption, ref double Zd, ref double Az, ref double RaR, ref double DecR)        {
            ...
            {
                Equ2Hor32(Jd_Ut1, DeltT, Accuracy, x, y, ref Location, Ra, Dec, RefOption, ref Zd, ref Az, ref RaR, ref DecR);
            }
        }
        ...
        public short Ter2Cel(double JdHigh, double JdLow, double DeltaT, Method Method, Accuracy Accuracy, OutputVectorOption OutputOption, double x, double y, double[] VecT, ref double[] VecC)
        {
            ...
            if (Is64Bit())
            {
                ...
                rc = Ter2Cel64(JdHigh, JdLow, DeltaT, Method, Accuracy, OutputOption, x, y, ref argVecT, ref VVecC);
            }
            else
            {
                var argVecT1 = ArrToPosVec(VecT);
                rc = Ter2Cel32(JdHigh, JdLow, DeltaT, Method, Accuracy, OutputOption, x, y, ref argVecT1, ref VVecC);
            }
        ...
        public short TopoStar(double JdTt, double DeltaT, CatEntry3 Star, OnSurface Position, Accuracy Accuracy, ref double Ra, ref double Dec)
        {
            if (Is64Bit())
            {
                return TopoStar64(JdTt, DeltaT, ref Star, ref Position, Accuracy, ref Ra, ref Dec);
            }
            else
            {
                return TopoStar32(JdTt, DeltaT, ref Star, ref Position, Accuracy, ref Ra, ref Dec);
            }
        }
        ...
        public void Wobble(double Tjd, double x, double y, double[] Pos1, ref double[] Pos2)        {
            ...
            {
                var argPos1 = ArrToPosVec(Pos1);
                Wobble64(Tjd, x, y, ref argPos1, ref VPos2);
            }
            else
            {
                var argPos11 = ArrToPosVec(Pos1);
                Wobble32(Tjd, x, y, ref argPos11, ref VPos2);
            }
        ...
        private short Ephem_Open(string Ephem_Name, ref double JD_Begin, ref double JD_End)
        {
            ...
            if (Is64Bit())
            {
                rc = EphemOpen64(Ephem_Name, ref JD_Begin, ref JD_End);
            }
            else
            {
                rc = EphemOpen32(Ephem_Name, ref JD_Begin, ref JD_End);
            }
            return rc;
        }
