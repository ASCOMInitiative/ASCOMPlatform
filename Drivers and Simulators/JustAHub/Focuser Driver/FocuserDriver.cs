...
namespace ASCOM.JustAHub
{
    ...
    public class Focuser : ReferenceCountedObjectBase, IFocuserV4, IDisposable
    {
        ...
        static Focuser()
        {
            ...
            attr = Attribute.GetCustomAttribute(typeof(Focuser), typeof(ServedClassNameAttribute));
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ASCOM.Focuser"/> class. Must be public to successfully register for COM.
        /// </summary>
        public Focuser()
        {
            try
            {
                ...
                FocuserHardware.Initialise();

                LogMessage("Focuser", "Starting driver initialisation");
                LogMessage("Focuser", $"ProgID: {ProgId}, Description: {ChooserDescription}");
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show($"{ex.Message}", "Exception creating ASCOM.JustAHub.Focuser", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        {
            Server.SetupDialog("Focuser");
            // Update the trace setting in case it was changed
            tl.Enabled = Settings.FocuserDriverLogging;
        }
        ...
        public bool Connected
        {
            get
            {
                try
                {
                    ...
                    bool connectedState = FocuserHardware.IsConnected(uniqueId);
            ...
            set
            {
                try
                {
                    ...
                    if (value) // Request to connect
                    {
                        ...
                        FocuserHardware.Connect(uniqueId, ConnectType.Connected);
                    }
                    else // Request to disconnect
                    {
                        LogMessage("Connected Set", "Disconnecting from device");
                        FocuserHardware.Disconnect(uniqueId, ConnectType.Connected);
                    }
                }
                catch (Exception ex)
                {
        ...
        public void Connect()
        {
            ...
            if (Common.DeviceInterfaces.DeviceCapabilities.HasConnectAndDeviceState(Common.DeviceTypes.Focuser, FocuserHardware.GetInterfaceVersion())) // Platform 7 or later device so Connect
            {
                LogMessage("Connect", "Issuing Connect command");
                FocuserHardware.Connect(uniqueId, ConnectType.Connect_Disconnect);
                return;
            }

            // Platform 6 or earlier so reject the method call
            LogMessage("Connect", "Throwing a MethodNotImplementedException because the connected Focuser is a Platform 6 or earlier device.");
            throw new MethodNotImplementedException($"Connect is not implemented in this IFocuserV{FocuserHardware.GetInterfaceVersion()} device.");
        }
        ...
        {
            // Check whether the connected device is Platform 7 or later
            if (Common.DeviceInterfaces.DeviceCapabilities.HasConnectAndDeviceState(Common.DeviceTypes.Focuser, FocuserHardware.GetInterfaceVersion())) // Platform 7 or later device so Disconnect
            {
                LogMessage("Disconnect", "Issuing Disconnect command");
                FocuserHardware.Disconnect(uniqueId, ConnectType.Connect_Disconnect);
                return;
            }

            // Platform 6 or earlier so reject the method call
            LogMessage("Disconnect", "Throwing a MethodNotImplementedException because the connected Focuser is a Platform 6 or earlier device.");
            throw new MethodNotImplementedException($"Disconnect is not implemented in this IFocuserV{FocuserHardware.GetInterfaceVersion()} device.");
        }
        ...
        public bool Connecting
        {
            ...
            {
                // Check whether the connected device is Platform 7 or later
                if (Common.DeviceInterfaces.DeviceCapabilities.HasConnectAndDeviceState(Common.DeviceTypes.Focuser, FocuserHardware.GetInterfaceVersion())) // Platform 7 or later device so call Connecting
                {
                    LogMessage("Connecting", "Calling Connecting property");
                    return FocuserHardware.Connecting;
                }

                // Platform 6 or earlier so reject the method call
                LogMessage("Connecting", "Throwing a PropertyNotImplementedException because the connected Focuser is a Platform 6 or earlier device.");
                throw new PropertyNotImplementedException($"Connecting is not implemented in this IFocuserV{FocuserHardware.GetInterfaceVersion()} device.");
            }
        }
        ...
        public IStateValueCollection DeviceState
        {
            get
            {
                ...
                if (Common.DeviceInterfaces.DeviceCapabilities.HasConnectAndDeviceState(Common.DeviceTypes.Focuser, FocuserHardware.GetInterfaceVersion())) // Platform 7 or later device so call Connecting
                {
                    ...
                    return FocuserHardware.DeviceState;
                }
