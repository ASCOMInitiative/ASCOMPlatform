...
namespace ASCOM.JustAHub
{
    ...
    public class ObservingConditions : ReferenceCountedObjectBase, IObservingConditionsV2, IDisposable
    {
        ...
        static ObservingConditions()
        {
            ...
            Attribute attr = Attribute.GetCustomAttribute(typeof(ObservingConditions), typeof(ProgIdAttribute));
            ...
            attr = Attribute.GetCustomAttribute(typeof(ObservingConditions), typeof(ServedClassNameAttribute));
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ASCOM.ObservingConditions"/> class. Must be public to successfully register for COM.
        /// </summary>
        public ObservingConditions()
        {
            try
            {
                ...
                ObservingConditionsHardware.Initialise();

                LogMessage("ObservingConditions", "Starting driver initialisation");
                LogMessage("ObservingConditions", $"ProgID: {ProgId}, Description: {ChooserDescription}");
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show($"{ex.Message}", "Exception creating ASCOM.JustAHub.ObservingConditions", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        // PUBLIC COM INTERFACE IObservingConditionsV3 IMPLEMENTATION
        ...
        {
            Server.SetupDialog("ObservingConditions");
            // Update the trace setting in case it was changed
            tl.Enabled = Settings.ObservingConditionsDriverLogging;
        }
        ...
        public bool Connected
        {
            get
            {
                try
                {
                    ...
                    bool connectedState = ObservingConditionsHardware.IsConnected(uniqueId);
            ...
            set
            {
                try
                {
                    ...
                    if (value) // Request to connect
                    {
                        ...
                        ObservingConditionsHardware.Connect(uniqueId, ConnectType.Connected);
                    }
                    else // Request to disconnect
                    {
                        LogMessage("Connected Set", "Disconnecting from device");
                        ObservingConditionsHardware.Disconnect(uniqueId, ConnectType.Connected);
                    }
                }
                catch (Exception ex)
                {
        ...
        public double AveragePeriod { get => GetValue(() => ObservingConditionsHardware.AveragePeriod); set => CallMember(() => ObservingConditionsHardware.AveragePeriod = value); }

        public double CloudCover { get => GetValue(() => ObservingConditionsHardware.CloudCover); }

        public double DewPoint { get => GetValue(() => ObservingConditionsHardware.DewPoint); }

        public double Humidity { get => GetValue(() => ObservingConditionsHardware.Humidity); }

        public double Pressure { get => GetValue(() => ObservingConditionsHardware.Pressure); }

        public double RainRate { get => GetValue(() => ObservingConditionsHardware.RainRate); }

        public double SkyBrightness { get => GetValue(() => ObservingConditionsHardware.SkyBrightness); }

        public double SkyQuality { get => GetValue(() => ObservingConditionsHardware.SkyQuality); }

        public double StarFWHM { get => GetValue(() => ObservingConditionsHardware.StarFWHM); }

        public double SkyTemperature { get => GetValue(() => ObservingConditionsHardware.SkyTemperature); }

        public double Temperature { get => GetValue(() => ObservingConditionsHardware.Temperature); }

        public double WindDirection { get => GetValue(() => ObservingConditionsHardware.WindDirection); }

        public double WindGust { get => GetValue(() => ObservingConditionsHardware.WindGust); }

        public double WindSpeed { get => GetValue(() => ObservingConditionsHardware.WindSpeed); }

        public double TimeSinceLastUpdate(string propertyName)
        {
            LogMessage("TimeSinceLastUpdate", $"Requested property: {propertyName}");
            return GetValue(() => ObservingConditionsHardware.TimeSinceLastUpdate(propertyName));
        }

        public string SensorDescription(string propertyName)
        {
            LogMessage("SensorDescription", $"Requested property: {propertyName}");
            return GetValue(() => ObservingConditionsHardware.SensorDescription(propertyName));
        }

        public void Refresh()
        {
            LogMessage("Refresh", $"Method called");
            CallMember(() => ObservingConditionsHardware.Refresh());
        }
        ...
        {
            if (!ObservingConditionsHardware.IsConnected(uniqueId))
            {
        ...
        public void Connect()
        {
            ...
            if (Common.DeviceInterfaces.DeviceCapabilities.HasConnectAndDeviceState(Common.DeviceTypes.ObservingConditions, ObservingConditionsHardware.GetInterfaceVersion())) // Platform 7 or later device so Connect
            {
                LogMessage("Connect", "Issuing Connect command");
                ObservingConditionsHardware.Connect(uniqueId, ConnectType.Connect_Disconnect);
                return;
            }

            // Platform 6 or earlier so reject the method call
            LogMessage("Connect", "Throwing a MethodNotImplementedException because the connected ObservingConditions is a Platform 6 or earlier device.");
            throw new MethodNotImplementedException($"Connect is not implemented in this IObservingConditionsV{ObservingConditionsHardware.GetInterfaceVersion()} device.");
        }
        ...
        {
            // Check whether the connected device is Platform 7 or later
            if (Common.DeviceInterfaces.DeviceCapabilities.HasConnectAndDeviceState(Common.DeviceTypes.ObservingConditions, ObservingConditionsHardware.GetInterfaceVersion())) // Platform 7 or later device so Disconnect
            {
                LogMessage("Disconnect", "Issuing Disconnect command");
                ObservingConditionsHardware.Disconnect(uniqueId, ConnectType.Connect_Disconnect);
                return;
            }

            // Platform 6 or earlier so reject the method call
            LogMessage("Disconnect", "Throwing a MethodNotImplementedException because the connected ObservingConditions is a Platform 6 or earlier device.");
            throw new MethodNotImplementedException($"Disconnect is not implemented in this IObservingConditionsV{ObservingConditionsHardware.GetInterfaceVersion()} device.");
        }
        ...
        public bool Connecting
        {
            ...
            {
                // Check whether the connected device is Platform 7 or later
                if (Common.DeviceInterfaces.DeviceCapabilities.HasConnectAndDeviceState(Common.DeviceTypes.ObservingConditions, ObservingConditionsHardware.GetInterfaceVersion())) // Platform 7 or later device so call Connecting
                {
                ...
                // Platform 6 or earlier so reject the method call
                LogMessage("Connecting", "Throwing a PropertyNotImplementedException because the connected ObservingConditions is a Platform 6 or earlier device.");
                throw new PropertyNotImplementedException($"Connecting is not implemented in this IObservingConditionsV{ObservingConditionsHardware.GetInterfaceVersion()} device.");
            }
        }
        ...
        public IStateValueCollection DeviceState
        {
            get
            {
                ...
                if (Common.DeviceInterfaces.DeviceCapabilities.HasConnectAndDeviceState(Common.DeviceTypes.ObservingConditions, ObservingConditionsHardware.GetInterfaceVersion())) // Platform 7 or later device so call Connecting
                {
                    ...
                    return ObservingConditionsHardware.DeviceState;
                }
