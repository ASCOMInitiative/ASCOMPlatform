...
{
    /// <summary>
    /// ASCOM JustAHub ObservingConditions main functional class shared by all instances of the driver class.
    /// </summary>
    ...
    {
#if DEBUG
        private static DriverAccess.ObservingConditions device; // ObservingConditions device being hosted
#else
        private static dynamic device; // ObservingConditions device being hosted
#endif
        ...
        static ObservingConditionsHardware()
        {
            try
            {
                ...
                TL = new TraceLogger("", $"JustAHub.ObservingConditions{(Environment.Is64BitProcess ? "64" : "")}.Proxy")
                {
                    Enabled = Settings.ObservingConditionsHardwareLogging
                };
            }
            catch (Exception ex)
            {
                try { LogMessage("JustAHub", $"Initialisation exception: {ex}"); } catch { }
                MessageBox.Show($"{ex.Message}", $"Exception creating {ObservingConditions.ChooserDescription} ({ObservingConditions.ProgId})", MessageBoxButtons.OK, MessageBoxIcon.Error);
                throw;
            }
        }
        ...
        internal static void Initialise()
        {
            ...
            if (!runOnce)
            {
                ...
                if (string.IsNullOrEmpty(Settings.ObservingConditionsHostedProgId))
                    throw new InvalidValueException("The configured ObservingConditions ProgID in JustAHub is null or empty");

                LogMessage("Initialise", $"Hosted ProgID: {Settings.ObservingConditionsHostedProgId}");
        ...
        public static void Connect(Guid uniqueId, ConnectType connectType)
        {
            ...
            // Test whether the ObservingConditions is already connected
            if (!device.Connected) // ObservingConditions hardware is not connected so connect
            {
                ...
                switch (connectType)
                {
                    case ConnectType.Connected:
                        ...
                        LogMessage("Connect", $"ObservingConditions connected OK.");
                        break;
        ...
        public static void Disconnect(Guid uniqueId, ConnectType connectType)
        {
            ...
            if (uniqueIds.Count == 0) // No instances remain connected so disconnect the ObservingConditions device
            {
                ...
                switch (connectType)
                {
                    case ConnectType.Connected:
                        ...
                        LogMessage("Disconnect", $"ObservingConditions disconnected OK.");
                        break;
        ...
        /// <summary>
        /// IObservingConditionsV4 and later Connecting property
        /// </summary>
        ...
        /// <summary>
        /// IObservingConditionsV4 and later DeviceState property
        /// </summary>
        ...
        public static void CreateInstance()
        {
            ...
            try
            {
                // Create an instance of the ObservingConditions
                try
                {
#if DEBUG
                    LogMessage("CreateInstance", $"Creating DriverAccess ObservingConditions device.");
                    device = new DriverAccess.ObservingConditions(Settings.ObservingConditionsHostedProgId);
#else
                    // Get the Type of this ProgID
                    Type type = Type.GetTypeFromProgID(Settings.ObservingConditionsHostedProgId);
                    LogMessage("CreateInstance", $"Created Type for ProgID: {Settings.ObservingConditionsHostedProgId} OK.");
                    device = Activator.CreateInstance(type);
#endif
                    LogMessage("CreateInstance", $"Created COM object for ProgID: {Settings.ObservingConditionsHostedProgId} OK.");
                }
                catch (Exception ex1)
                {
                    throw new InvalidOperationException($"Unable to create an instance of the ObservingConditions with ProgID {Settings.ObservingConditionsHostedProgId}: {ex1.Message}");
                }
            }
            catch (Exception ex)
            {
                throw new InvalidOperationException($"Unable to create Type for ProgID {Settings.ObservingConditionsHostedProgId}: {ex.Message}");
            }
        }
        ...
        public static void Dispose()
        {
            if (!(device is null))
            {
                ...
                try { LogMessage("JustAHub.Dispose", $"Disposed DriverAccess ObservingConditions object."); } catch { }
                try { device = null; } catch (Exception) { }
#else
                try { Marshal.ReleaseComObject(device); } catch (Exception) { }
                try { LogMessage("JustAHub.Dispose", $"Released ObservingConditions COM object."); } catch { }
                try { device = null; } catch (Exception) { }
#endif
            }
        ...
        public static double AveragePeriod { get => device.AveragePeriod; set => device.AveragePeriod = value; }

        public static double CloudCover { get => device.CloudCover; }

        public static double DewPoint { get => device.DewPoint; }

        public static double Humidity { get => device.Humidity; }

        public static double Pressure { get => device.Pressure; }

        public static double RainRate { get => device.RainRate; }

        public static double SkyBrightness { get => device.SkyBrightness; }

        public static double SkyQuality { get => device.SkyQuality; }

        public static double StarFWHM { get => device.StarFWHM; }

        public static double SkyTemperature { get => device.SkyTemperature; }

        public static double Temperature { get => device.Temperature; }

        public static double WindDirection { get => device.WindDirection; }

        public static double WindGust { get => device.WindGust; }

        public static double WindSpeed { get => device.WindSpeed; }
        ...
        internal static int GetInterfaceVersion()
        {
            ...
            if (device.Connected) // ObservingConditions is connected so save the value for future use
            {
                interfaceVersion = InterfaceVersion;
                return interfaceVersion.Value;
            }
            else // ObservingConditions is not connected so return the reported value but don't save it
            {
                return iVersion;
            }
        }
