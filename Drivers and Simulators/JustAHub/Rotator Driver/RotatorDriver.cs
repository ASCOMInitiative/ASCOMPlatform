...
namespace ASCOM.JustAHub
{
    ...
    public class Rotator : ReferenceCountedObjectBase, IRotatorV4, IDisposable
    {
        ...
        static Rotator()
        {
            ...
            attr = Attribute.GetCustomAttribute(typeof(Rotator), typeof(ServedClassNameAttribute));
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ASCOM.Rotator"/> class. Must be public to successfully register for COM.
        /// </summary>
        public Rotator()
        {
            try
            {
                ...
                RotatorHardware.Initialise();

                LogMessage("Rotator", "Starting driver initialisation");
                LogMessage("Rotator", $"ProgID: {ProgId}, Description: {ChooserDescription}");
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show($"{ex.Message}", "Exception creating ASCOM.JustAHub.Rotator", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        {
            Server.SetupDialog("Rotator");
            // Update the trace setting in case it was changed
            tl.Enabled = Settings.RotatorDriverLogging;
        }
        ...
        public bool Connected
        {
            get
            {
                try
                {
                    ...
                    bool connectedState = RotatorHardware.IsConnected(uniqueId);
            ...
            set
            {
                try
                {
                    ...
                    if (value) // Request to connect
                    {
                        ...
                        RotatorHardware.Connect(uniqueId, ConnectType.Connected);
                    }
                    else // Request to disconnect
                    {
                        LogMessage("Connected Set", "Disconnecting from device");
                        RotatorHardware.Disconnect(uniqueId, ConnectType.Connected);
                    }
                }
                catch (Exception ex)
                {
        ...
        public bool IsMoving => GetValue(() => RotatorHardware.IsMoving);

        public float Position => GetValue(() => RotatorHardware.Position);

        public bool Reverse { get => GetValue(() => RotatorHardware.Reverse); set => CallMember(() => RotatorHardware.Reverse = value); }

        public float StepSize => GetValue(() => RotatorHardware.StepSize);

        public float TargetPosition => GetValue(() => RotatorHardware.TargetPosition);

        public bool CanReverse => GetValue(() => RotatorHardware.CanReverse);

        public float MechanicalPosition => GetValue(() => RotatorHardware.MechanicalPosition);

        public void Halt()
        {
            CallMember(() => RotatorHardware.Halt());
        }

        public void Move(float Position)
        {
            CallMember(() => RotatorHardware.Move(Position));
        }

        public void MoveAbsolute(float Position)
        {
            CallMember(() => RotatorHardware.MoveAbsolute(Position));
        }

        public void MoveMechanical(float Position)
        {
            CallMember(() => RotatorHardware.MoveMechanical(Position));
        }

        public void Sync(float Position)
        {
            CallMember(() => RotatorHardware.Sync(Position));
        }
        ...
        {
            if (!RotatorHardware.IsConnected(uniqueId))
            {
        ...
        public void Connect()
        {
            ...
            if (Common.DeviceInterfaces.DeviceCapabilities.HasConnectAndDeviceState(Common.DeviceTypes.Rotator, RotatorHardware.GetInterfaceVersion())) // Platform 7 or later device so Connect
            {
                LogMessage("Connect", "Issuing Connect command");
                RotatorHardware.Connect(uniqueId, ConnectType.Connect_Disconnect);
                return;
            }

            // Platform 6 or earlier so reject the method call
            LogMessage("Connect", "Throwing a MethodNotImplementedException because the connected Rotator is a Platform 6 or earlier device.");
            throw new MethodNotImplementedException($"Connect is not implemented in this IRotatorV{RotatorHardware.GetInterfaceVersion()} device.");
        }
        ...
        {
            // Check whether the connected device is Platform 7 or later
            if (Common.DeviceInterfaces.DeviceCapabilities.HasConnectAndDeviceState(Common.DeviceTypes.Rotator, RotatorHardware.GetInterfaceVersion())) // Platform 7 or later device so Disconnect
            {
                LogMessage("Disconnect", "Issuing Disconnect command");
                RotatorHardware.Disconnect(uniqueId, ConnectType.Connect_Disconnect);
                return;
            }

            // Platform 6 or earlier so reject the method call
            LogMessage("Disconnect", "Throwing a MethodNotImplementedException because the connected Rotator is a Platform 6 or earlier device.");
            throw new MethodNotImplementedException($"Disconnect is not implemented in this IRotatorV{RotatorHardware.GetInterfaceVersion()} device.");
        }
        ...
        public bool Connecting
        {
            ...
            {
                // Check whether the connected device is Platform 7 or later
                if (Common.DeviceInterfaces.DeviceCapabilities.HasConnectAndDeviceState(Common.DeviceTypes.Rotator, RotatorHardware.GetInterfaceVersion())) // Platform 7 or later device so call Connecting
                {
                    LogMessage("Connecting", "Calling Connecting property");
                    return RotatorHardware.Connecting;
                }

                // Platform 6 or earlier so reject the method call
                LogMessage("Connecting", "Throwing a PropertyNotImplementedException because the connected Rotator is a Platform 6 or earlier device.");
                throw new PropertyNotImplementedException($"Connecting is not implemented in this IRotatorV{RotatorHardware.GetInterfaceVersion()} device.");
            }
        }
        ...
        public IStateValueCollection DeviceState
        {
            get
            {
                ...
                if (Common.DeviceInterfaces.DeviceCapabilities.HasConnectAndDeviceState(Common.DeviceTypes.Rotator, RotatorHardware.GetInterfaceVersion())) // Platform 7 or later device so call Connecting
                {
                    ...
                    return RotatorHardware.DeviceState;
                }
