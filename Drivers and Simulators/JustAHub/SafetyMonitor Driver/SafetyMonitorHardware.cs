    ...
    internal static class SafetyMonitorHardware
    {
        ...
        static SafetyMonitorHardware()
        {
            try
            {
                ...
                TL = new TraceLogger("", $"JustAHub.SafetyMonitor{(Environment.Is64BitProcess ? "64" : "")}.Proxy")
                {
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show($"{ex.Message}", $"Exception creating {SafetyMonitor.ChooserDescription} ({SafetyMonitor.ProgId})", MessageBoxButtons.OK, MessageBoxIcon.Error);
                throw;
            }
        }
