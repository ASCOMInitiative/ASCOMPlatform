...
namespace ASCOM.JustAHub
{
    ...
    public class Switch : ReferenceCountedObjectBase, ISwitchV3, IDisposable
    {
        ...
        static Switch()
        {
            ...
            Attribute attr = Attribute.GetCustomAttribute(typeof(Switch), typeof(ProgIdAttribute));
            ...
            attr = Attribute.GetCustomAttribute(typeof(Switch), typeof(ServedClassNameAttribute));
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ASCOM.Switch"/> class. Must be public to successfully register for COM.
        /// </summary>
        public Switch()
        {
            try
            {
                ...
                SwitchHardware.Initialise();

                LogMessage("Switch", "Starting driver initialisation");
                LogMessage("Switch", $"ProgID: {ProgId}, Description: {ChooserDescription}");
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show($"{ex.Message}", "Exception creating ASCOM.JustAHub.Switch", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        {
            Server.SetupDialog("Switch");
            // Update the trace setting in case it was changed
            tl.Enabled = Settings.SwitchDriverLogging;
        }
        ...
        public bool Connected
        {
            get
            {
                try
                {
                    ...
                    bool connectedState = SwitchHardware.IsConnected(uniqueId);
            ...
            set
            {
                try
                {
                    ...
                    if (value) // Request to connect
                    {
                        ...
                        SwitchHardware.Connect(uniqueId, ConnectType.Connected);
                    }
                    else // Request to disconnect
                    {
                        LogMessage("Connected Set", "Disconnecting from device");
                        SwitchHardware.Disconnect(uniqueId, ConnectType.Connected);
                    }
                }
                catch (Exception ex)
                {
        ...
        public short MaxSwitch => GetValue(() => SwitchHardware.MaxSwitch);

        public string GetSwitchName(short id)
        {
            return GetValue(() => SwitchHardware.GetSwitchName(id));
        }

        public void SetSwitchName(short id, string name)
        {
            CallMember(() => SwitchHardware.SetSwitchName(id,name));
        }

        public bool GetSwitch(short id)
        {
            return GetValue(() => SwitchHardware.GetSwitch(id));
        }

        public void SetSwitch(short id, bool state)
        {
            CallMember(() => SwitchHardware.SetSwitch(id, state));
        }

        public bool CanWrite(short id)
        {
            return GetValue(() => SwitchHardware.CanWrite(id));
        }

        public string GetSwitchDescription(short id)
        {
            return GetValue(() => SwitchHardware.GetSwitchDescription(id));
        }

        public double GetSwitchValue(short id)
        {
            return GetValue(() => SwitchHardware.GetSwitchValue(id));
        }

        public double MaxSwitchValue(short id)
        {
            return GetValue(() => SwitchHardware.MaxSwitchValue(id));
        }

        public double MinSwitchValue(short id)
        {
            return GetValue(() => SwitchHardware.MinSwitchValue(id));
        }

        public void SetSwitchValue(short id, double value)
        {
            CallMember(() => SwitchHardware.SetSwitchValue(id, value));
        }

        public double SwitchStep(short id)
        {
            return GetValue(() => SwitchHardware.SwitchStep(id));
        }

        public void SetAsync(short id, bool state)
        {
            CallMember(() => SwitchHardware.SetAsync(id, state));
        }

        public void SetAsyncValue(short id, double value)
        {
            CallMember(() => SwitchHardware.SetAsyncValue(id, value));
        }

        public bool CanAsync(short id)
        {
            return GetValue(() => SwitchHardware.CanAsync(id));
        }

        public bool StateChangeComplete(short id)
        {
            return GetValue(() => SwitchHardware.StateChangeComplete(id));
        }

        public void CancelAsync(short id)
        {
            CallMember(() => SwitchHardware.CancelAsync(id));
        }
        ...
        {
            if (!SwitchHardware.IsConnected(uniqueId))
            {
        ...
        public void Connect()
        {
            ...
            if (Common.DeviceInterfaces.DeviceCapabilities.HasConnectAndDeviceState(Common.DeviceTypes.Switch, SwitchHardware.GetInterfaceVersion())) // Platform 7 or later device so Connect
            {
                LogMessage("Connect", "Issuing Connect command");
                SwitchHardware.Connect(uniqueId, ConnectType.Connect_Disconnect);
                return;
            }

            // Platform 6 or earlier so reject the method call
            LogMessage("Connect", "Throwing a MethodNotImplementedException because the connected Switch is a Platform 6 or earlier device.");
            throw new MethodNotImplementedException($"Connect is not implemented in this ISwitchV{SwitchHardware.GetInterfaceVersion()} device.");
        }
        ...
        {
            // Check whether the connected device is Platform 7 or later
            if (Common.DeviceInterfaces.DeviceCapabilities.HasConnectAndDeviceState(Common.DeviceTypes.Switch, SwitchHardware.GetInterfaceVersion())) // Platform 7 or later device so Disconnect
            {
                LogMessage("Disconnect", "Issuing Disconnect command");
                SwitchHardware.Disconnect(uniqueId, ConnectType.Connect_Disconnect);
                return;
            }

            // Platform 6 or earlier so reject the method call
            LogMessage("Disconnect", "Throwing a MethodNotImplementedException because the connected Switch is a Platform 6 or earlier device.");
            throw new MethodNotImplementedException($"Disconnect is not implemented in this ISwitchV{SwitchHardware.GetInterfaceVersion()} device.");
        }
        ...
        public bool Connecting
        {
            ...
            {
                // Check whether the connected device is Platform 7 or later
                if (Common.DeviceInterfaces.DeviceCapabilities.HasConnectAndDeviceState(Common.DeviceTypes.Switch, SwitchHardware.GetInterfaceVersion())) // Platform 7 or later device so call Connecting
                {
                    LogMessage("Connecting", "Calling Connecting property");
                    return SwitchHardware.Connecting;
                }

                // Platform 6 or earlier so reject the method call
                LogMessage("Connecting", "Throwing a PropertyNotImplementedException because the connected Switch is a Platform 6 or earlier device.");
                throw new PropertyNotImplementedException($"Connecting is not implemented in this ISwitchV{SwitchHardware.GetInterfaceVersion()} device.");
            }
        }
        ...
        public IStateValueCollection DeviceState
        {
            get
            {
                ...
                if (Common.DeviceInterfaces.DeviceCapabilities.HasConnectAndDeviceState(Common.DeviceTypes.Switch, SwitchHardware.GetInterfaceVersion())) // Platform 7 or later device so call Connecting
                {
                    ...
                    return SwitchHardware.DeviceState;
                }
