...
namespace ASCOM.JustAHub
{
    ...
    public class Dome : ReferenceCountedObjectBase, IDomeV3, IDisposable
    {
        ...
        static Dome()
        {
            ...
            Attribute attr = Attribute.GetCustomAttribute(typeof(Dome), typeof(ProgIdAttribute));
            ...
            attr = Attribute.GetCustomAttribute(typeof(Dome), typeof(ServedClassNameAttribute));
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ASCOM.Dome"/> class. Must be public to successfully register for COM.
        /// </summary>
        public Dome()
        {
            try
            {
                ...
                DomeHardware.Initialise();

                LogMessage("Dome", "Starting driver initialisation");
                LogMessage("Dome", $"ProgID: {ProgId}, Description: {ChooserDescription}");
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show($"{ex.Message}", "Exception creating ASCOM.JustAHub.Dome", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        {
            Server.SetupDialog("Dome");
            // Update the trace setting in case it was changed
            tl.Enabled = Settings.DomeDriverLogging;
        }
        ...
        public bool Connected
        {
            get
            {
                try
                {
                    ...
                    bool connectedState = DomeHardware.IsConnected(uniqueId);
            ...
            set
            {
                try
                {
                    ...
                    if (value) // Request to connect
                    {
                        ...
                        DomeHardware.Connect(uniqueId, ConnectType.Connected);
                    }
                    else // Request to disconnect
                    {
                        LogMessage("Connected Set", "Disconnecting from device");
                        DomeHardware.Disconnect(uniqueId, ConnectType.Connected);
                    }
                }
                catch (Exception ex)
                {
        ...
        public double Altitude => GetValue(() => DomeHardware.Altitude);

        public bool AtHome => GetValue(() => DomeHardware.AtHome);

        public bool AtPark => GetValue(() => DomeHardware.AtPark);

        public double Azimuth => GetValue(() => DomeHardware.Azimuth);

        public bool CanFindHome => GetValue(() => DomeHardware.CanFindHome);

        public bool CanPark => GetValue(() => DomeHardware.CanPark);

        public bool CanSetAltitude => GetValue(() => DomeHardware.CanSetAltitude);

        public bool CanSetAzimuth => GetValue(() => DomeHardware.CanSetAzimuth);

        public bool CanSetPark => GetValue(() => DomeHardware.CanSetPark);

        public bool CanSetShutter => GetValue(() => DomeHardware.CanSetShutter);

        public bool CanSlave => GetValue(() => DomeHardware.CanSlave);

        public bool CanSyncAzimuth => GetValue(() => DomeHardware.CanSyncAzimuth);

        public ShutterState ShutterStatus => GetValue(() => DomeHardware.ShutterStatus);

        public bool Slaved { get => GetValue(() => DomeHardware.Slaved); set => CallMember(() => DomeHardware.Slaved = value); }

        public bool Slewing => GetValue(() => DomeHardware.Slewing);

        public void AbortSlew()
        {
            CallMember(() => DomeHardware.AbortSlew());
        }

        public void CloseShutter()
        {
            CallMember(() => DomeHardware.CloseShutter());
        }

        public void FindHome()
        {
            CallMember(() => DomeHardware.FindHome());
        }

        public void OpenShutter()
        {
            CallMember(() => DomeHardware.OpenShutter());
        }

        public void Park()
        {
            CallMember(() => DomeHardware.Park());
        }

        public void SetPark()
        {
            CallMember(() => DomeHardware.SetPark());
        }

        public void SlewToAltitude(double Altitude)
        {
            CallMember(() => DomeHardware.SlewToAltitude(Altitude));
        }

        public void SlewToAzimuth(double Azimuth)
        {
            CallMember(() => DomeHardware.SlewToAzimuth(Azimuth));
        }

        public void SyncToAzimuth(double Azimuth)
        {
            CallMember(() => DomeHardware.SyncToAzimuth(Azimuth));
        }
        ...
        {
            if (!DomeHardware.IsConnected(uniqueId))
            {
        ...
        public void Connect()
        {
            ...
            if (Common.DeviceInterfaces.DeviceCapabilities.HasConnectAndDeviceState(Common.DeviceTypes.Dome, DomeHardware.GetInterfaceVersion())) // Platform 7 or later device so Connect
            {
                LogMessage("Connect", "Issuing Connect command");
                DomeHardware.Connect(uniqueId, ConnectType.Connect_Disconnect);
                return;
            }

            // Platform 6 or earlier so reject the method call
            LogMessage("Connect", "Throwing a MethodNotImplementedException because the connected Dome is a Platform 6 or earlier device.");
            throw new MethodNotImplementedException($"Connect is not implemented in this IDomeV{DomeHardware.GetInterfaceVersion()} device.");
        }
        ...
        {
            // Check whether the connected device is Platform 7 or later
            if (Common.DeviceInterfaces.DeviceCapabilities.HasConnectAndDeviceState(Common.DeviceTypes.Dome, DomeHardware.GetInterfaceVersion())) // Platform 7 or later device so Disconnect
            {
                LogMessage("Disconnect", "Issuing Disconnect command");
                DomeHardware.Disconnect(uniqueId, ConnectType.Connect_Disconnect);
                return;
            }

            // Platform 6 or earlier so reject the method call
            LogMessage("Disconnect", "Throwing a MethodNotImplementedException because the connected Dome is a Platform 6 or earlier device.");
            throw new MethodNotImplementedException($"Disconnect is not implemented in this IDomeV{DomeHardware.GetInterfaceVersion()} device.");
        }
        ...
        public bool Connecting
        {
            ...
            {
                // Check whether the connected device is Platform 7 or later
                if (Common.DeviceInterfaces.DeviceCapabilities.HasConnectAndDeviceState(Common.DeviceTypes.Dome, DomeHardware.GetInterfaceVersion())) // Platform 7 or later device so call Connecting
                {
                    LogMessage("Connecting", "Calling Connecting property");
                    return DomeHardware.Connecting;
                }

                // Platform 6 or earlier so reject the method call
                LogMessage("Connecting", "Throwing a PropertyNotImplementedException because the connected Dome is a Platform 6 or earlier device.");
                throw new PropertyNotImplementedException($"Connecting is not implemented in this IDomeV{DomeHardware.GetInterfaceVersion()} device.");
            }
        }
        ...
        public IStateValueCollection DeviceState
        {
            get
            {
                ...
                if (Common.DeviceInterfaces.DeviceCapabilities.HasConnectAndDeviceState(Common.DeviceTypes.Dome, DomeHardware.GetInterfaceVersion())) // Platform 7 or later device so call Connecting
                {
                    ...
                    return DomeHardware.DeviceState;
                }
