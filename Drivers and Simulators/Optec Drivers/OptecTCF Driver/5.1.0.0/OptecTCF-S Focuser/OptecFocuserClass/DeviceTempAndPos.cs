    ...
    partial class OptecFocuser
    {
        ...
        public static double GetTemperature()
        {
            try
            {
                ...
                else if (_ConnectionState == ConnectionStates.SerialMode)
                {
                    if (DeviceSettings.TempProbePresent)
                    {
                        string resp = SendCommandGetResponse("FTEMPx", "T=", 800, 4);
        ...
        public static void MoveFocus(int NewPos)
        {
            try
            {
                ...
                if (NewPos < 0 || NewPos > DeviceSettings.MaxStep)
                {
                    throw new ASCOM.InvalidValueException("MoveFocus()", NewPos.ToString(),
                        "0 thru " + DeviceSettings.MaxStep);
                }
