        ...
        public bool Link
        {
            get
            {
                ...
                if (CheckDevice(false, false))
                {
                    ...
                    {
                        retval = Service.Link;
                        LogActivityLine(msgType, $"Get Link flag - {retval}");
                    }
                    catch (Exception xcp)
                    {
                        LogActivityLine(msgType, $"Get Link flag - {Failed}");
                        LogActivityLine(msgType, $"{xcp}");
            ...
            set
            {
                ...
                try
                {
                    ...
                    if (value != isConnected)
                    {
                        ...
                        if (value == isConnected)
                        {
                            LogActivityLine(msgType, $"Set Link flag -> {value} {(value ? "(connected)" : "(disconnected)")}");
                            LogActivityLine(msgType, "{0} took {1} milliseconds."
                                            , (value) ? "Connection" : "Disconnection"
                                            , i * msDelay);
                        }
                        ...
                        {
                            LogActivityLine(msgType, $"Set Link flag -> {value} {Failed}");
                            LogActivityLine(msgType, "{0} failed after {1} milliseconds"
                                            , (value) ? "Connection" : "Disconnection"
                                            , numTries * msDelay);
                        }
                    ...
                    {
                        LogActivityLine(msgType, $"Set Link flag -> {value} (no change)");
                    }
                }
                catch (Exception xcp)
                {
