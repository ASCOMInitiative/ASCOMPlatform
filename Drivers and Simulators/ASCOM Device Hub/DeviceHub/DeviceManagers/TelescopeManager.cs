...
{
    public partial class TelescopeManager : DeviceManagerBase, ITelescopeManager, ITelescopeV4, IDisposable
    {
        ...
        public bool ConnectTelescope()
        {
            ...
            return ConnectTelescope(TelescopeID, false);
        }

        public bool ConnectTelescope(string scopeID, bool interactiveConnect = true)
        {
            ConnectError = "";
            ...
            if (IsConnected)
            {
                try
                {
                    ...
                    task.Wait(); // This will propagate any un-handled exception

                }
                catch (AggregateException xcp)
                {
                    LogActivityLine(ActivityMessageTypes.Other, "Attempting to initialize telescope operation caught an un-handled exception. Details follow:");
        ...
        public void DisconnectTelescope(bool interactiveDisconnect = false)
        {
            if (!DeviceCreated)
            {
                return;
        ...
        {
            LogAppMessage($"Method entered", "BeginSlewToCoordinatesAsync");
            ...
            try
            {
                ...
                LogAppMessage($"Sending Slew Message", "BeginSlewToCoordinatesAsync");
                SendSlewMessage(ra, dec);
                LogAppMessage($"Calling SlewToCoordinatesAsync", "BeginSlewToCoordinatesAsync");
                SlewToCoordinatesAsync(ra, dec);
                LogAppMessage($"SlewToCoordinatesAsync returned OK", "BeginSlewToCoordinatesAsync");
                slewed = true;
            ...
            {
                LogAppMessage($"Exception: {xcp.Message}", "BeginSlewToCoordinatesAsync");
                throw xcp;
            }
            finally
            {
            ...
            LogAppMessage($"Method exited", "BeginSlewToCoordinatesAsync");
            LogAppMessage($"", "BeginSlewToCoordinatesAsync");
        }
