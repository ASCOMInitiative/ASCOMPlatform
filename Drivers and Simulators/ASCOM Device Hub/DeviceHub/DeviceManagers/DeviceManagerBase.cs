...
namespace ASCOM.DeviceHub
{
    ...
    public class DeviceManagerBase : NormalizedValueBase, INotifyPropertyChanged
    {
        ...
        protected T GetServiceProperty<T>(Func<T> getCmd, T defaultValue, ActivityMessageTypes messageType = ActivityMessageTypes.Status, [System.Runtime.CompilerServices.CallerMemberName] string propName = "???")
        {
            ...
            if (xcp == null)
            {
                ...
                else if (retval.GetType() == typeof(Double))
                {
                    switch (propName)
                    {
                        case "Altitude":
                        ...
                        case "TargetDeclination":
                            LogActivityLine(messageType, $"Get {propName}: {Convert.ToDouble(retval).ToDMS()}");
                            break;

                        case "RightAscension":
                        case "SiderealTime":
                        case "TargetRightAscension":
                            LogActivityLine(messageType, $"Get {propName}: {Convert.ToDouble(retval).ToHMS()}");
                            break;

                        default:
                            LogActivityLine(messageType, $"Get {propName}: {retval}");
                            break;
                    }
                }
                else
        ...
        protected void SetFastPolling(string caller = "")
        {
            ...
            if (PollingPeriod != newInterval)
            {
                ...
                LogActivityLine(ActivityMessageTypes.Other, $"Started fast polling{fromCaller} every {PollingPeriod} ms.");
            }
            else
            {
                LogActivityLine(ActivityMessageTypes.Other, $"SetFastPolling called{fromCaller} when already fast polling.");
            }
        }
