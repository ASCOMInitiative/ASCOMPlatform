...
namespace ASCOM.Simulator
{
    ...
    public class ClassFactory : IClassFactory
    {
        ...
        {
            Hub.TL.LogMessage("ClassFactory", "Start of initialisation");
            ...
            {
                Hub.TL.LogMessage("ClassFactory", "Adding Type: " + T.FullName);
                m_InterfaceTypes.Add(T);
            }
            Hub.TL.LogMessage("ClassFactory", "Completed initialisation");
        }
        ...
        public bool RegisterClassObject()
        {
            ...
            Hub.TL.LogMessage("RegisterClassObject", "GUID: " + m_ClassId.ToString() + ", Cookie: " + m_Cookie);
            return (i == 0);
        }

        public bool RevokeClassObject()
        {
            Hub.TL.LogMessage("RevokeClassObject", m_Cookie.ToString());
        ...
        {
            Hub.TL.LogMessage("ResumeClassObjects", "Called");
        ...
        {
            Hub.TL.LogMessage("SuspendClassObjects", "Called");
        ...
        {
            Hub.TL.LogMessage("CreateInstance", "GUID: " + riid.ToString());
        ...
        {
            Hub.TL.LogMessage("LockServer", "Lock server: " + bLock);
            if (bLock)
                Server.CountLock();
            else
                Server.UncountLock();
            // Always attempt to see if we need to shutdown this server application.
            Hub.TL.LogMessage("LockServer", "Calling ExitIf to check whether we can exit the server");
            Server.ExitIf();
        }
        #endregion
    }
