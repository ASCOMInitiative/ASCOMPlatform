...
namespace ASCOM.Simulator
{
    public static class Server
    {
        ...
        public static int UncountObject()
        {
            ...
            OCSimulator.TL.LogMessage("UncountObject", "Decrementing object count to: " + (objsInUse - 1));
        ...
        public static int CountLock()
        {
            ...
            OCSimulator.TL.LogMessage("CountLock", "Incrementing lock count to: " + (serverLocks + 1));
        ...
        public static int UncountLock()
        {
            ...
            OCSimulator.TL.LogMessage("UncountLock", "Decrementing lock count to: " + (serverLocks - 1));
            ...
            lock (lockObject)
            {
                ...
                if ((ObjectsCount <= 0) && (ServerLockCount <= 0))
                {
                    if (StartedByCOM)
                    {
                        ...
                        OCSimulator.TL.LogMessage("ExitIf", "WM_QUIT outcome. Succcess: " + success + ", return code: " + returnCode.ToString("X"));
                    }
                }
            }
        }
        ...
        private static void ElevateSelf(string arg)
        {
            ...
            {
                MessageBox.Show("The Observing Conditions Simulator was not " + (arg == "/register" ? "registered" : "unregistered") +
                    " because you did not allow it.", "Observing Conditions Simulator", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.ToString(), "Observing Conditions Simulator", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
            return;
        }
        ...
        private static void RegisterObjects()
        {
            ...
            catch (Exception ex)
            {
                MessageBox.Show("Error while registering the server:\n" + ex.ToString(),
                        "Observing Conditions Simulator", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                return;
            }
            finally
            {
                ...
                switch (args[0].ToLower())
                {
                    ...
                    default:
                        MessageBox.Show("Unknown argument: " + args[0] + "\nValid are : -register, -unregister and -embedding",
                            "Observing Conditions Simulator", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        break;
                }
            }
            else
