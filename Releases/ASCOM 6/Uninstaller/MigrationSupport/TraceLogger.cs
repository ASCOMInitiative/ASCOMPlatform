...
/// </remarks>
    ...
    /// <para>This call enables automatic logging and sets the file type to "Default".</para></remarks>
        public TraceLogger() : base()
        {
            ...
            g_DefaultLogFilePath = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + Global.TRACE_LOGGER_PATH + Global.TRACE_LOGGER_FILENAME_BASE + Strings.Format(DateTime.Now, Global.TRACE_LOGGER_FILE_NAME_DATE_FORMAT);
    ...
    /// </remarks>
        ...
        private void CreateLogFile()
        {
            ...
            switch (g_LogFileName ?? "")
            {
                ...
                case Global.SERIAL_AUTO_FILENAME:
                    {
                    ...
                    case 13: // Either translate or respect CRLf depending on the setting of the respect parameter
                        {
                            if (p_RespectCrLf)
                            {
                                l_Msg += Strings.Mid(p_Msg, i, 1);
                            }
                            else
                            {
                                l_Msg += "[" + Strings.Right("00" + Conversion.Hex(CharNo), 2) + "]";
                            }
        ...
        private void LogMsgFormatter(string p_Test, string p_Msg, bool p_NewLine, bool p_RespectCrLf)
        {
            ...
            {
                Global.LogEvent("LogMsgFormatter", "Exception", EventLogEntryType.Error, Global.EventLogErrors.TraceLoggerException, ex.ToString());
        ...
        private void GetTraceLoggerMutex(string Method, string Parameters)
        {
            ...
            try
            {
                ...
                GotMutex = mut.WaitOne(Global.PROFILE_MUTEX_TIMEOUT, false);
            }
            ...
            catch (System.Threading.AbandonedMutexException ex)
            {
                ...
                Global.LogEvent("TraceLogger", "AbandonedMutexException in " + Method + ", parameters: " + Parameters, EventLogEntryType.Error, Global.EventLogErrors.TraceLoggerMutexAbandoned, ex.ToString());
                if (Global.GetBool(Global.ABANDONED_MUTEXT_TRACE, Global.ABANDONED_MUTEX_TRACE_DEFAULT))
                {
                    Global.LogEvent("TraceLogger", "AbandonedMutexException in " + Method + ": Throwing exception to application", EventLogEntryType.Warning, Global.EventLogErrors.TraceLoggerMutexAbandoned, null);
                    throw; // Throw the exception in order to report it
                }
                else
                {
                    Global.LogEvent("TraceLogger", "AbandonedMutexException in " + Method + ": Absorbing exception, continuing normal execution", EventLogEntryType.Warning, Global.EventLogErrors.TraceLoggerMutexAbandoned, null);
                    GotMutex = true;
            ...
            if (!GotMutex)
            {
                Global.LogEvent(Method, "Timed out waiting for TraceLogger mutex in " + Method + ", parameters: " + Parameters, EventLogEntryType.Error, Global.EventLogErrors.TraceLoggerMutexTimeOut, null);
