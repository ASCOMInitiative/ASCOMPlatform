...
namespace TEMPLATENAMESPACE
{
	...
	public class ClassFactory : IClassFactory
	{
		...
		public ClassFactory(Type type)
		{
            ...
            m_ClassType = type;
            m_progid = type.Assembly.FullName;                  // get the progid from the assemble FullName so multiple devices can be used
            int i = m_progid.IndexOf(',');
            m_progid = m_progid.Substring(0, i);
		...
		void IClassFactory.CreateInstance(IntPtr pUnkOuter, ref Guid riid, out IntPtr ppvObject)
		{
            ...
            // try to use the multiple device constructor, if it fails use the original one.
            //
			foreach (Type iType in m_InterfaceTypes)
			{
				if (riid == Marshal.GenerateGuidForType(iType))
				{
                    try
                    {
                        ppvObject = Marshal.GetComInterfaceForObject(Activator.CreateInstance(m_ClassType, new object[] { m_progid }), iType);
                    }
			...
			if (riid == IID_IDispatch)
			{
                try
                {
                    ppvObject = Marshal.GetIDispatchForObject(Activator.CreateInstance(m_ClassType, new object[] { m_progid }));
                }
                catch (MissingMethodException)
                {
			...
			else if(riid == IID_IUnknown)
			{
                try
                {
                    ppvObject = Marshal.GetIUnknownForObject(Activator.CreateInstance(m_ClassType, new object[] { m_progid }));
                }
                catch (MissingMethodException)
                {
