...
//
// 13-March-11 cdr  (1.2.0) modify to allow multiple drivers of the same type to be registered.
//
using System;
...
{
	public static class TEMPLATEDEVICENAME
	{
...
// Stores the main thread's thread id.
        private static int s_iObjsInUse;						// Keeps a count on the total number of objects alive.
		private static int s_iServerLocks;						// Keeps a lock count on this application.
// True if server started by COM (-embedding)
		...
		private static string s_sAppId = "{$guid1$}";	// Our AppId
		...
		public static int CountObject()
		{
			...
			return Interlocked.Increment(ref s_iObjsInUse);
		}
		...
		public static int UncountObject()
		{
			...
			return Interlocked.Decrement(ref s_iObjsInUse);
		}
		...
		public static int CountLock()
		{
			...
			return Interlocked.Increment(ref s_iServerLocks);
		}
		...
		public static int UncountLock()
		{
			...
			return Interlocked.Decrement(ref s_iServerLocks);
		}
		...
		private static bool LoadComObjectAssemblies()
		{
			...
			foreach (FileInfo fi in d.GetFiles("*.dll"))
			{
                ...
                i = fqClassName.LastIndexOf('-');
                if (i > 0)
                    fqClassName = fqClassName.Substring(0, i);          // remove trailing -n
                //
				...
				try
				{
					...
					{
						s_ComObjectTypes.Add(so.GetType(fqClassName, true));
		...
		private static void RegisterObjects()
		{
			...
			try
			{
				...
				// HKCR\APPID\appid
				//
                using (RegistryKey key = Registry.ClassesRoot.CreateSubKey("APPID\\" + s_sAppId))
                {
                    key.SetValue(null, assyDescription);
                    key.SetValue("AppID", s_sAppId);
			...
			foreach (Type type in s_ComObjectTypes)
			{
				...
				try
				{
                    ...
                    string deviceType = progid.Substring(progid.LastIndexOf('.') + 1);
                    progid = type.Assembly.FullName;
		...
		private static void UnregisterObjects()
		{
			...
			// Local server's DCOM/AppID information
			//
			Registry.ClassesRoot.DeleteSubKey(String.Format("APPID\\{0}", s_sAppId), false);
