...
//
// FACILITY:	StarryNight V5 Plug-In DLL ASCOM Telescope Control
//
// ABSTRACT:	
//
// USING:		
...
//				Developed under Microsoft Visual C++ Version 7
//
...
//						was bad, but...
// 07-Oct-05	MK		Minor bug fixes and improvements (mostly related to
//						expanded telescope interface)
// 23-Nov-05	MK		Added bV10Flag and made sure that RefreshTelescopePanel 
//						and SlewLimits are called only if SNP V5 and above is used
//========================================================================
...
bool _bScopeCanMoveAxis		= false;	// This is true if scope can move in a specified direction
...
static bool _bDoIdleBusy	= false;	// This is true if scope is currently performing scheduled tasks
...
bool _bV10Plugin		= false; // true if running under SNP V5
...
NOARGPROC _psn_RefreshPanel;
CBPBPROC _psn_GetAreCoordinatesWithinUserSlewLimits;
...
static short initPlugin(PInitialize* p)
{
	...
	//the following are specific to V10Plugin
	_psn_RefreshPanel = (NOARGPROC)GetProcAddress(_hProc, "sn_RefreshTelescopePanel");
	_psn_GetAreCoordinatesWithinUserSlewLimits = (CBPBPROC)GetProcAddress(_hProc, "sn_GetAreCoordinatesWithinUserSlewLimits");	
	...
	if(_psn_SetPluginStatusText == NULL)	// Running under SNP V3?
	{
		...
		_bV10Plugin = false;				// We can NOT use sn_RefreshTelescopePanel and other V10 features
	}
	else
	{
		_bV8Plugin = true;					// We can use sn_SetPluginStatusText

		if (_psn_RefreshPanel == NULL) {	
			p->fPluginAPI = kVersion8Plugin;	// This is a V8 or later plugin (has new status string stuff)
			_bV10Plugin = false;				// We can NOT use sn_RefreshTelescopePanel
		}
		else
		{
			p->fPluginAPI = kVersion10Plugin;	// This is a V10 or later plugin (has telescope panel stuff)
			_bV10Plugin = true;				// We can use sn_RefreshTelescopePanel
		}
	}
	...
	// the following has been removed as it can be rather awkward at times (03/2005 - MK)
...
{
	MessageBox(NULL, 
"Starry Night v5.7.2 plug-in for ASCOM telescopes. For more information on ASCOM \
...
static void doMenuStatus(PMenuStatus *p)
{
	...
	switch(p->fMenuCommand)
	{
		...
		case cmd_TelescopeSlew:
			if (_bScopeCanPark && isParked)
				...
				p->fIsEnabled = _bScopeCanSlew && _bScopeActive && !_bIsSlewing;
									//enable if scope can slew, it is connected and it is not currently slewing
			break;
		case cmd_TelescopeSync:
			if (_bScopeCanPark && isParked)
				...
				p->fIsEnabled = _bScopeCanSync && _bScopeActive && !_bIsSlewing;
								//enable if scope can sync, it is connected and it is not currently slewing
			break;
		case cmd_TelescopeClose:
			p->fIsEnabled = _bScopeActive && !_bIsSlewing;
								//enable if scope is connected but not slewing
			break;
		...
		case cmd_TelescopePark:
			if (_bScopeCanPark && (!_bIsSlewing))
				p->fIsEnabled = (!isParked && _bScopeActive); //enable if scope is connected but not parked
			break;
		case cmd_TelescopeUnpark:
			if(_bScopeCanUnpark)
				p->fIsEnabled = (isParked && _bScopeActive); //enable if scope is connected and parked
			break;
		case cmd_TelescopeSetParkPosition:
			if (_bScopeCanSetPark && (!_bIsSlewing))
				p->fIsEnabled = (!isParked && _bScopeActive); //enable if scope is connected and not parked
			break;
		case cmd_TelescopeGoHome:
			if (_bScopeCanFindHome && (!_bIsSlewing))
			{
				if (_bScopeCanPark && isParked)
					p->fIsEnabled = false; // disable if parked
				else
					p->fIsEnabled = (!isParked && _bScopeActive);
							//enable if scope is connected, it can home and it is not parked nor slewing
			}
			break;
		default:
			break;
...
static short doIdle(PEvent *p)
{
	...
	{
		if(_bV8Plugin && !_bOfflineMsgSent)			// Fill in status box on SNP V4 and above
		{
...
/*
// THIS MESSES UP DURING DAYLIGHT SAVINGS TIME...
...
*/
	...
	if((t > tn))	//&& (p->fInWindow == _pWnd))			// If update interval has elapsed
	{
		__try
		{
			...
			//If the telescope was slewing and it's status changed then refresh the panel
			if (_bIsSlewing && (!IsSlewing()))
			{
				_bIsSlewing = false; //changed the slewing status flag since staus has just changed
				if (_bV10Plugin)
					(*_psn_RefreshPanel)();
			}

			//update parked information
			...
			else
			{
				...
				{
					//make sure that while scope is being asked for info no other queries are taking place
					_bScopeActive = false;
				...
				{
					if ((_dScopeRA == coordinate_NotAvailable) && (_dScopeDec == coordinate_NotAvailable))
					{
						cToPstr("Cannot communicate with the telescope", (unsigned char*)&buf);
						cToPstr( "Couldn't get the telescope position! Check your system to make sure no cables got unplugged, your interface is turned on, and your batteries are okay.", (unsigned char*)&buf2);
...
//						_dGazeFOVRad = w /  p->fPixelsPerRadian;
			...
			case op_DrawIndicator:					// i/o PIndicator*
				...
				if(_bScopeActive &&	_bOurIndicator && (_dScopeRA != coordinate_NotAvailable) && (_dScopeDec != coordinate_NotAvailable))// &&
			...
			case op_TelescopeMotion:				// PScopeMotion*
				if (((PScopeMotion *)ioParams)->fMotionDirection == direction_NONE_STOP) {
					...
					if(_bScopeCanMoveAxis)
					{
						_bScopeBusy = true;				// Stop doIdle()
						outErr = SlewScope( (PScopeMotion*) ioParams );
						SetForegroundWindow(_hWndMain);		// Bring ourself to the front
					}
				}
				break;
