...
//
// FACILITY:	StarryNight V5Plug-In DLL ASCOM Telescope Control
//
// ABSTRACT:	
//
// USING:		
...
"Failed to read the driver ID from the registry.",
		...
		_bScopeCanMoveAxis = GetCanMoveAxis();		// Can it move in a specified direction?
...
void TermScope(void)
{
	...
	_bScopeCanMoveAxis  = false;	// This is true if scope can move in a specified direction
...
// ------
//
bool GetCanMoveAxis(void)
{
	OLECHAR *name = L"CanMoveAxis";
	DISPID dispid;
    DISPPARAMS dispParms;
    VARIANTARG rgvarg[1];
    EXCEPINFO excep;
	VARIANT vRes;
	bool result = false;									// Assume success (our retval)
	HRESULT hr;
	char *cp;
	char buf[256];
	...
	result = (vRes.boolVal == VARIANT_TRUE);			// Return C++ bool result
	...
	return (result && (vRes.boolVal == VARIANT_TRUE));
}
...
bool IsSlewing(void)
{
	...
	if(FAILED(_p_DrvDisp->GetIDsOfNames(
									...
									LOCALE_USER_DEFAULT,
									&dispid)))
		drvFail( "The ASCOM scope driver is missing the Slewing property.", NULL, true);
...
//	----
//
//	Slew the telescope in a specified direction
//
short SlewScope( PScopeMotion* inMotionParam )
{
	OLECHAR *name;
	DISPID dispid;
    DISPPARAMS dispParms;
	...
	__try
	{
		...
		_bIsSlewing = true; // set isSlewing flag since slewing is about to start

		//for SN V5 and up make sure that the new state is updated (all things that should be off
		// while slewing is in process need to be set to off on the panel)
		if (_bV10Plugin)
			(*_psn_RefreshPanel)();
		...
		if ((*inMotionParam).fMotionDirection == direction_NONE_STOP) {
			rgvarg[0].iVal = 0;
			rgvarg[1].dblVal = 0;
			if(FAILED(hr = _p_DrvDisp->Invoke(
										dispid,
										...
										NULL)))
			{
				...
				// Report the error and let the driver continue.
				//
				drvFail("Moving telescope in direction failed.", &excep, false);
			}
			...
			rgvarg[0].iVal = 1 - (((short)(*inMotionParam).fMotionDirection - 1) % 2);
			rgvarg[1].dblVal = (*inMotionParam).fRelativeSlewRate;
			if(FAILED(hr = _p_DrvDisp->Invoke(
										dispid,
										IID_NULL, 
										LOCALE_USER_DEFAULT, 
