...
namespace ASCOM.Controls
{
	///<summary>
	///  <para>para>
	///    The Anunciator control provides a simple, standard method of displaying status notifications to the user within a Windows Forms application.
	///    Anunciators are best used with the companiion <see cref = "AnunciatorPanel" /> control, although they can be placed anywhere on a Windows Form.
	...
	///    an <see cref = "AnunciatorPanel" />) and is not directly settable by the user.
	///  </para>
	///</summary>
	public sealed class Anunciator : Label
	{
		/// <summary>
		...
		/// </summary>
		private static readonly List<Anunciator> UpdateList = new List<Anunciator>();
		...
		private static void tmrCadence_Tick(object sender, EventArgs e)
		{
			...
			lock (UpdateList)
			{
				...
				foreach (Anunciator item in UpdateList)
				{
					try
					{
