...
#Region "Class Factory"
	...
	Friend Class VBSampleClassFactory
		Inherits ClassFactoryBase

		Public Overrides Sub CreateInstance(ByVal pUnkOuter As Guid, ByRef riid As Guid, ByRef ppvObject As IntPtr)

			LocalServerCOM.Trace("CSharpSampleClassFactory.CreateInstance().")
			LocalServerCOM.Trace("Requesting Interface: " + riid.ToString())

			If (riid = Marshal.GenerateGuidForType(IAscomSample.GetType)) Or _
			 (riid = LocalServerCOM.IID_IDispatch) Or _
			 (riid = LocalServerCOM.IID_IUnknown) Then
				ppvObject = Marshal.GetComInterfaceForObject(new CSharpSample(), typeof(IAscomSample));
			Else
				Throw New COMException("No interface", unchecked(&H80004002))
			End If
