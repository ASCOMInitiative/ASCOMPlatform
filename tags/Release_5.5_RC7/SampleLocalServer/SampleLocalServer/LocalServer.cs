//
// SampleLocalServer Local COM Server
//
...
{
	public class SampleLocalServer
	{
		...
		private static string m_sAppId = "{349B7C26-B353-415d-8FCF-CA0017AD0FC3}";	// Our AppId
		#endregion
		...
		// the SampleLocalServer\bin\Debug subfolder.
		//
		private static bool LoadComObjectAssemblies()
		{
			...
			int i = assyPath.LastIndexOf(@"\SampleLocalServer\bin\");						// Look for us running in IDE
			if (i == -1) i = assyPath.LastIndexOf('\\');
			assyPath = assyPath.Remove(i, assyPath.Length - i) + "\\SampleLocalServerServedClasses";
			...
			foreach (FileInfo fi in d.GetFiles("*.dll"))
			{
				...
				catch (Exception e)
				{
					MessageBox.Show("Failed to load served COM class assembly " + fi.Name + " - " + e.Message,
						"SampleLocalServer", MessageBoxButtons.OK, MessageBoxIcon.Stop);
					return false;
				}
				...
				switch (args[0].ToLower())
				{
					...
					default:
						MessageBox.Show("Unknown argument: " + args[0] + "\nValid are : -register, -unregister and -embedding",
							"SampleLocalServer", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
						break;
				}
			}
			else
