...
namespace ASCOM.OptecTCF_S
{
    ...
    public class ComRegistration : Installer
    {
        ...
        public override void Install(System.Collections.IDictionary stateSaver)
        {
            ...
            //Check for the beta version and uninstall it
            string path = "C:/Program Files/Common Files/ASCOM/Focuser/ASCOM.OptecTCF_Driver.Focuser.dll";
            if (File.Exists(path))
            {
                Logger.TLogger.LogMessage("Installer", "A version of this driver has been found on the machine");
                FileVersionInfo fileinfo = FileVersionInfo.GetVersionInfo(path);
                if (fileinfo.ProductName.Contains("Beta3"))
                {
                    Logger.TLogger.LogMessage("Installer", "Beta Version Found On Machine");
                    File.Delete(path);
                }
                else
                {
                    Logger.TLogger.LogMessage("Installer", "Beta Version NOT Detected On Machine");
                }
            }
            else
            {
                Logger.TLogger.LogMessage("Installer", "Beta Version NOT Found On Machine");
            }
