...
/// </summary>
    ...
    public enum ASM_CMP_FLAGS
    {
        ...
        CULTURE = 0x40,
        CUSTOM = 0x80,
        ALL = NAME | MAJOR_VERSION | MINOR_VERSION | REVISION_NUMBER | BUILD_NUMBER | PUBLIC_KEY_TOKEN | CULTURE | CUSTOM,
        DEFAULT = 0x100
    }
...
/// </summary>
    public enum ASM_NAME
    {
        ...
        ASM_NAME_HASH_VALUEField,
        ASM_NAME_NAMEField,
        ASM_NAME_MAJOR_VERSIONField,
        ASM_NAME_MINOR_VERSIONField,
        ASM_NAME_BUILD_NUMBERField,
        ASM_NAME_REVISION_NUMBERField,
        ASM_NAME_CULTUREField,
        ASM_NAME_PROCESSOR_ID_ARRAYField,
        ASM_NAME_OSINFO_ARRAYField,
        ASM_NAME_HASH_ALGIDField,
        ASM_NAME_ALIASField,
        ASM_NAME_CODEBASE_URLField,
        ASM_NAME_CODEBASE_LASTMODField,
        ASM_NAME_NULL_PUBLIC_KEYField,
        ASM_NAME_NULL_PUBLIC_KEY_TOKENField,
        ASM_NAME_CUSTOMField,
        ASM_NAME_NULL_CUSTOMField,
        ASM_NAME_MVIDField,
        ASM_NAME_MAX_PARAMSField
    }
...
/// </summary>
    public enum IASSEMBLYCACHE_UNINSTALL_DISPOSITION
    {
        ...
        IASSEMBLYCACHE_UNINSTALL_DISPOSITION_UNINSTALLEDField = 1,
        IASSEMBLYCACHE_UNINSTALL_DISPOSITION_STILL_IN_USEField = 2,
        IASSEMBLYCACHE_UNINSTALL_DISPOSITION_ALREADY_UNINSTALLEDField = 3,
        IASSEMBLYCACHE_UNINSTALL_DISPOSITION_DELETE_PENDINGField = 4,
        IASSEMBLYCACHE_UNINSTALL_DISPOSITION_HAS_INSTALL_REFERENCESField = 5,
        IASSEMBLYCACHE_UNINSTALL_DISPOSITION_REFERENCE_NOT_FOUNDField = 6
    }
...
/// </summary>
    ...
    /// <param name="dwReserved">must be 0</param>
        ...
        private static extern void CreateAssemblyCache(ref IAssemblyCache ppAsmCache, uint dwReserved);
    ...
    /// <param name="pvReserved"> Must be null.</param>
        ...
        private static extern void CreateAssemblyNameObject(ref IAssemblyName ppAssemblyNameObj, string szAssemblyName, uint dwFlags, IntPtr pvReserved);
    ...
    /// <param name="pvReserved">Must be NULL.</param>
        [DllImport("fusion.dll", SetLastError = true, PreserveSig = false)]
        private static extern void CreateAssemblyEnum(ref IAssemblyEnum pEnum, IntPtr pUnkReserved, IAssemblyName pName, ASM_CACHE_FLAGS dwFlags, IntPtr pvReserved);
    ...
    /// <param name="pvReserved">Must be null.</param>
        [DllImport("fusion.dll", SetLastError = true, PreserveSig = false)]
        private static extern void CreateInstallReferenceEnum(ref IInstallReferenceEnum ppRefEnum, IAssemblyName pName, uint dwFlags, IntPtr pvReserved);
    ...
    /// <returns>IAssemblyCache COM interface</returns>
        ...
        public static IAssemblyName CreateAssemblyName(string name)
        {
            ...
            CreateAssemblyNameObject(ref an, name, 2U, (IntPtr)0);
        ...
        {
            var major = default(uint);
            var minor = default(uint);
            name.GetVersion(ref major, ref minor);
            return new Version((int)(major >> 16), (int)(major & 0xFFFFL), (int)(minor >> 16), (int)(minor & 0xFFFFL));
        }
        ...
        public static byte[] GetPublicKeyToken(IAssemblyName name)
        {
            ...
            name.GetProperty(ASM_NAME.ASM_NAME_PUBLIC_KEY_TOKENField, buffer, ref bufferSize);
            for (int i = 0; i <= 7; i++)
        ...
        {
            uint bufferSize = 512U;
            var buffer = Marshal.AllocHGlobal((int)bufferSize);
            name.GetProperty(ASM_NAME.ASM_NAME_PUBLIC_KEYField, buffer, ref bufferSize);
            byte[] result = new byte[(int)(bufferSize - 1L + 1)];
            for (int i = 0, loopTo = (int)(bufferSize - 1L); i <= loopTo; i++)
                result[i] = Marshal.ReadByte(buffer, i);
            Marshal.FreeHGlobal(buffer);
            return result;
        }
        ...
        public static CultureInfo GetCulture(IAssemblyName name)
        {
            uint bufferSize = 255U;
            var buffer = Marshal.AllocHGlobal((int)bufferSize);
            name.GetProperty(ASM_NAME.ASM_NAME_CULTUREField, buffer, ref bufferSize);
        ...
        {
            IAssemblyEnum ae = null;

            CreateAssemblyEnum(ref ae, (IntPtr)0, null, ASM_CACHE_FLAGS.ASM_CACHE_GAC, (IntPtr)0);
    ...
    /// <returns>0 if the enumeration is not at its end</returns>
        public static int GetNextAssembly(IAssemblyEnum enumerator, ref IAssemblyName name)
        {
            return enumerator.GetNextAssembly((IntPtr)0, ref name, 0U);
        }
        ...
        public static RemoveOutcome RemoveGAC(string AssemblyName)
        {
            var retval = new RemoveOutcome();

            var pCache = CreateAssemblyCache();
            // Get an IAssemblyCache interface
            var puldisposition = IASSEMBLYCACHE_UNINSTALL_DISPOSITION.IASSEMBLYCACHE_UNINSTALL_DISPOSITION_UNKNOWNField;
            // Initialise variable
            ...
            int result = pCache.UninstallAssembly(0U, AssemblyName, null, ref puldisposition);
...
/// </summary>
    ...
    /// 	</remarks>
        [PreserveSig()]
        int UninstallAssembly(uint dwFlags, [MarshalAs(UnmanagedType.LPWStr)] string pszAssemblyName, [MarshalAs(UnmanagedType.LPArray)] FusionInstallReference[] pRefData, ref IASSEMBLYCACHE_UNINSTALL_DISPOSITION pulDisposition);
    ...
    /// <returns></returns>
        [PreserveSig()]
        int CreateAssemblyCacheItem(uint dwFlags, IntPtr pvReserved, ref IAssemblyCacheItem ppAsmItem, [MarshalAs(UnmanagedType.LPWStr)] string pszAssemblyName);
    ...
    /// <returns></returns>
        [PreserveSig()]
        int CreateAssemblyScavenger([MarshalAs(UnmanagedType.IUnknown)] ref object ppAsmScavenger);
    ...
    /// <returns></returns>
        [PreserveSig()]
        int InstallAssembly(uint dwFlags, [MarshalAs(UnmanagedType.LPWStr)] string pszManifestFilePath, [MarshalAs(UnmanagedType.LPArray)] FusionInstallReference[] pRefData);
    }
...
/// </summary>
    ...
    /// <remarks>http://msdn.microsoft.com/library/default.asp?url=/library/en-us/cpguide/html/cpcondefaultmarshalingforstrings.asp</remarks>
        [PreserveSig()]
        int GetDisplayName([MarshalAs(UnmanagedType.LPWStr)] StringBuilder szDisplayName, ref uint pccDisplayName, ASM_DISPLAY_FLAGS dwDisplayFlags);
    ...
    /// <returns></returns>
        [PreserveSig()]
        int BindToObject(ref Guid refIID, [MarshalAs(UnmanagedType.IUnknown)] object pUnkSink, [MarshalAs(UnmanagedType.IUnknown)] object pUnkContext, [MarshalAs(UnmanagedType.LPWStr)] string szCodeBase, long llFlags, IntPtr pvReserved, uint cbReserved, ref IntPtr ppv);
    ...
    /// <remarks>http://msdn.microsoft.com/library/default.asp?url=/library/en-us/cpguide/html/cpcondefaultmarshalingforstrings.asp</remarks>
        [PreserveSig()]
        int GetName(ref uint lpcwBuffer, [MarshalAs(UnmanagedType.LPWStr)] StringBuilder pwzName);
    ...
    /// <returns></returns>
        [PreserveSig()]
        int GetVersion(ref uint pdwVersionHi, ref uint pdwVersionLow);
    ...
    /// <returns></returns>
        [PreserveSig()]
        int Clone(ref IAssemblyName pName);
    }
...
/// </summary>
    ...
    /// <returns></returns>
        [PreserveSig()]
        int GetNextAssembly(IntPtr pvReserved, ref IAssemblyName ppName, uint dwFlags);
    ...
    /// <returns></returns>
        [PreserveSig()]
        int Clone(ref IAssemblyEnum ppEnum);
    }
...
/// </summary>
    ...
    /// <returns></returns>
        [PreserveSig()]
        int GetReference([MarshalAs(UnmanagedType.LPArray)] ref FusionInstallReference[] ppRefData, uint dwFlags, IntPtr pvReserved);
    }
...
/// </summary>
    ...
    /// <returns></returns>
        [PreserveSig()]
        int GetNextInstallReferenceItem(ref IInstallReferenceItem ppRefItem, uint dwFlags, IntPtr pvReserved);
    }
...
/// </summary>
    ...
    /// <param name="puliMaxSize"></param>
        ...
        void CreateStream(uint dwFlags, [MarshalAs(UnmanagedType.LPWStr)] string pszStreamName, uint dwFormat, uint dwFormatFlags, ref IStream ppIStream, ref long puliMaxSize);
    ...
    /// <param name="pulDisposition"></param>
        void Commit(uint dwFlags, ref long pulDisposition);
